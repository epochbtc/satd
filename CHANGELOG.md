...
- A full mempool raises a rolling minimum fee (Core's `mempool min fee not met`), reported as `getmempoolinfo.mempoolminfee`; `sendrawtransaction` and `testmempoolaccept` rejections use Core's `reason, detail` message; `getrawmempool` accepts `mempool_sequence=true`.
