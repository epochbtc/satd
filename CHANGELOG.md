...
- `-uacomment` is honored instead of accepted and ignored. It appends comments
  ...
  user agent over 256 bytes, both with Core's wording. Beyond losing the
  operator's label, ignoring it made every satd node on a host advertise an
  identical `subversion`.
