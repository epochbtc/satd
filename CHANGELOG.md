...
- `sat-tui` services row gains an `sp-idx` column with tweak-index status and
  backfill progress; shown only when the index is enabled. `getindexinfo`
  now reports `silentpayments.enabled` alongside `synced`, and
  `silentpayments.backfill.progress_ratio` (walk-start-based, matching the
  Prometheus gauge) so consumers need not misreconstruct progress from the
  cursor/snapshot heights.
