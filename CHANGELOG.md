...
- `invalidateblock` now moves the node's header tip off the invalidated branch, as Core's `RecalculateBestHeader` does, and `reconsiderblock` lets it back. `getblockchaininfo` reported a `headers` height on the refused branch — one the node would never reach, growing with every block mined on it (#778).
