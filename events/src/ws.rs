...
/// Envelope-shaped JSON for a watch match. Re-exported from `node` so this
/// carrier and the alert webhook dispatcher render the same bytes for the same
/// match — the shape is documented once, in `docs/api/streaming.md`.
use node::events::watch_match_json;
