...
//! the backfill produce byte-identical rows to a from-genesis sync. That is
//! asserted, not assumed: the whole `sp_tweaks` column family a live node
//! built across a reorg is compared against the one a backfill rebuilt over
//! the same chain, by
//! `test_sp_index_backfill_rebuilds_live_rows_byte_identically` in
//! `satd/tests/regtest.rs`. On regtest taproot activates at
...
impl BackfillRunner {
    ...
    /// Current progress over the walked span, as the ETA's rate meter
    /// measures it.
    fn progress_ratio(&self) -> f64 {
        self.handle.cursor().progress_ratio(self.walk_start())
    }
    ...
    pub fn run(self) -> Result<(), BackfillError> {
        ...
        // starts walking — rather than at `start()` time. Held across the
        // whole walk so every exit path clears it (#546).
