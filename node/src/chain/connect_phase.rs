...
/// Distinct phases of `connect_preprocessed_block`. Ordered roughly by
/// the sequence the connector visits them in.
///
/// `Idle` is the resting state — set by the connector after `TipWrite`
/// completes, so a watchdog that fires when the connector is between
/// blocks sees `idle` instead of a stale earlier phase.
...
mod tests {
    ...
    fn from_index_maps_round_trip() {
        for v in 0..ConnectPhase::COUNT {
            ...
            assert_eq!(phase as usize, v.min(ConnectPhase::COUNT - 1).min(9));
        }
