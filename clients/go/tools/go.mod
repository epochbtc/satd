// Pinned build tools for the Go SDK: the protobuf/gRPC code generators, the
// buf compiler that drives them, and the two linters the Lint job runs.
//
// This is a SEPARATE module on purpose. Tool dependencies declared in the SDK's
// own go.mod would enter every consumer's module graph and could force MVS
// version bumps (golang.org/x/tools, the whole buf tree) on applications that
// only import the client. Keeping them here means the published SDK graph stays
// exactly gRPC + protobuf.
//
// Invoke through `go tool` from this directory - see gen.sh and the Lint job.
module github.com/epochbtc/satd/clients/go/tools
