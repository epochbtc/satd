...
| `connect`, `addnode`, `seednode` | Newly-added peers are registered and dialed immediately; existing connections are untouched. Removing an entry does **not** disconnect that peer (use `disconnectnode`), matching Core's `-addnode`. |
