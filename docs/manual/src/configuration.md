...
| `--rpctlsbind=<addr:port>` | None | Enables native TLS for JSON-RPC; no TLS-terminating sidecar is needed. Requires `--rpctlscert` and `--rpctlskey`. A non-empty `--rpcallowip` gates this listener too. |
