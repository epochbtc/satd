...
- **MCP is now HTTP(S)-only with native TLS; the stdio transport is removed.**
  The MCP server is part of the running `satd` process, so the stdio transport
  (which launched a second, datadir-locking node) made little sense — `-mcpstdio`
  and the stdio serve path are gone. The Streamable HTTP listener gains native
  TLS via the same `tls_config` layer as the RPC/Esplora/Electrum surfaces:
  `-mcpcert`/`-mcpkey` enable **HTTPS**, and `-mcpmtls` (+ `-mcpmtlsclientca`,
  optional `-mcpmtlsclientallow`) adds mutual TLS. TLS hot-reloads on `SIGUSR1`.
  The listener is also hardened to match the RPC TLS surface: a concurrent-
  connection cap, a bounded TLS-handshake and request-header timeout (slowloris
  guards), accept-error backoff, and graceful drain of in-flight requests on
  shutdown.
