...
- **`-loglevel`** — set global verbosity (`trace`/`debug`/`info`/`warn`/`error`)
  ...
  `tracing` env-filter, so a bare level affects all targets; Core scopes a bare
  level to `-debug`-enabled categories, so this is a slight, documented semantic
  difference. Hot-reloadable on `SIGHUP`, alongside `-debug`/`-debugexclude`.
