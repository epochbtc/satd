...
- **Fix testnet/testnet4 min-difficulty difficulty computation to walk parent
  ...
  *not* a 20-minute min-difficulty block; finding it means walking back over the
  ...
  uses a by-height lookup (the boundary block is well-defined). Found live on a
  testnet4 dogfood node frozen at a height whose next real-difficulty block sat
  just past a one-block index gap; a regression test reproduces the gap.
