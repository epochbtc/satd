...
- **Fix a sync wedge on a competing same-height fork at the IBD connect
  ...
  *other* N), the scheduler counted height N as "stored" and reported itself
  ...
  testnet4 dogfood node, wedged at a tip fork.) On a persistent `bad-prevblk`
  failure the connector now checks whether a higher-work competing header chain
  exists (`best_header_beats_active_tip`); if so it tears down the stalled
  scheduler and hands off to the reorg-capable steady-state path, which pulls
  the missing fork block (`missing_blocks_for_best_header_chain`) and runs the
  reorg — the same recovery a restart performs, without the restart.
  Non-fork persistent failures keep the prior fail-and-restart behavior.
