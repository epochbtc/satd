...
- **`-blocknotify=<cmd>`** — run a shell command on each new best block, with
  `%s` replaced by the block hash (Bitcoin Core parity). Each invocation is
  detached, so a slow hook never stalls the event loop or block connection;
  non-zero exits and spawn failures are logged, not fatal.
