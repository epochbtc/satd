...
`streamprevoutmeta = hash` there is no mempool-input value to test, so an
`AddScripts` carrying `min_values` is rejected. The matched value never appears
in the wire event — the floor is purely a delivery filter.
