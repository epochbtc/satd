...
[`docs/api/streaming.md`](../api/streaming.md) §7.6.
...
- **On-demand `reload()`, via atomic `SetWatchSet`.** The companion to the
  ...
  shrinking the live set. This replaces an earlier client-computed `Add*`/`Remove*` delta
...
See [`docs/api/streaming.md`](../api/streaming.md) §11.1.
