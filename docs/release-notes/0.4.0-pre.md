...
- **No server-dependency leak.** The wire types were extracted into a thin
  ...
  `prost`, `tokio`, `tokio-stream`, `thiserror`, optional `bitcoin`) — never the
  `node` crate or RocksDB. The SDK versions on the additive `v1` wire schema, not
  the node's release cadence. Credentials are redacted from `Debug`; the build
  does not yet terminate TLS, so a bearer token over plaintext `http://` is
  cleartext (use loopback or a TLS proxy until native TLS lands).
