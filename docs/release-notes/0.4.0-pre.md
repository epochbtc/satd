...
- **Watch-set loader (optional).** Integrators whose watch-set has a durable
  ...
  unchanged.
- **On-demand `reload()`.** The companion to the connect-time loader for truth
  that changes *while the stream is up* (a bulk import, an admin rotation, an
  operator reconciliation). `ResilientWatch::reload()` re-runs the loader, diffs
  the freshly-loaded set against the live watch-set, and applies **only the
  delta** on the wire — `Add*` before `Remove*` / `RemoveDescriptor` so coverage
  never drops to nothing, identical items untouched. It reuses the backoff /
  cursor-anchor / loader plumbing instead of dropping and rebuilding the wrapper.
  Returns a `ReloadSummary { added, removed, unchanged, applied }`; a
  disconnected reload defers to the next reconnect's loader rather than erroring.
  (Descriptor removal in the diff relies on the new `RemoveDescriptor` server
  support, also in this release.)
