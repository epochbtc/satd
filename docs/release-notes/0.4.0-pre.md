...
block source is a no-op, and concurrent re-anchors are bounded to one in flight per
stream. WS/SSE clients (no mid-stream control channel) continue to re-anchor by
reconnecting with `?from_cursor=`. See [`docs/api/streaming.md`](../api/streaming.md)
§7.3.1.
