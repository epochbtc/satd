...
### MCP metrics snapshot matches the HTTP scrape

The MCP `get_metrics_snapshot` tool — a one-shot render of the Prometheus
`/metrics` body for ad-hoc inspection — hardcoded the address-index gauges to
zero, so on a node running `addressindex=1` it reported
`satd_addrindex_enabled 0` and `satd_addrindex_subscriptions_active 0` even
while the real HTTP `/metrics` endpoint reported the true values. The tool now
renders both from the node's live config and subscription registry, so the
MCP snapshot and the HTTP scrape agree. The HTTP `/metrics` listener was always
correct; this only affected the MCP convenience tool.
