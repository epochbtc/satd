...
- **Bitcoin Core interop canary** (`scripts/canary/core-interop-smoke.sh`) — the only canary that tests satd against the reference implementation itself. Peers satd with a real `bitcoind` (`lncm/bitcoind`) regtest node and asserts bidirectional P2P interop: handshake + peer identity, **BIP 324 v2 encrypted transport** negotiation, block sync in **both** directions (satd↔Core), and tx relay in **both** directions. Catches consensus/relay/wire drift vs. Core directly rather than via a downstream's tolerance. Surfaced and now guards two satd P2P bugs: `getdata MSG_CMPCT_BLOCK` was silently dropped (so satd-mined blocks never reached a Core peer that requested the tip as a compact block), and `sendrawtransaction`-submitted txs were never announced to peers (so RPC broadcasts never propagated to the network).
