...
CRATE_SOURCES = {"bdk_wallet": BDK_LOCK}
...
def check() -> int:
    ...
    for hold in holds:
        if hold.removeprefix("HOLD_") not in pins:
...
def freshness(now: dt.datetime) -> list[dict]:
    ...
    for crate, lock in CRATE_SOURCES.items():
        tracked.append((crate, locked_version(lock, crate), crate_releases(crate)))
